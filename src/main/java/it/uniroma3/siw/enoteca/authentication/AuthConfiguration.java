package it.uniroma3.siw.enoteca.authentication;
...
import static it.uniroma3.siw.enoteca.model.Credentials.ADMIN_ROLE;
import static it.uniroma3.siw.enoteca.model.Credentials.DEFAULT_ROLE;
...
public class AuthConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .antMatchers(HttpMethod.GET, "/", "/home", "/alcolici", "/alcolico/{id}", "/caseProduttrici", "/casaProduttrice/{id}", "/tipologie", "/tipologia/{id}", "/nazioni", "/nazione/{id}", "/login", "/register", "/css/**", "/img/**").permitAll()
                // chiunque (autenticato o no) può mandare richieste POST al punto di accesso per login e register 
                .antMatchers(HttpMethod.POST, "/login", "/register", "/searchAlcolico").permitAll()
                // solo gli utenti autenticati con ruolo ADMIN possono accedere a risorse con path /admin/**
                .antMatchers(HttpMethod.GET, "/admin/**").hasAnyAuthority(ADMIN_ROLE)
                .antMatchers(HttpMethod.POST, "/admin/**").hasAnyAuthority(ADMIN_ROLE)
                .antMatchers(HttpMethod.GET, "/wishlist").hasAnyAuthority(DEFAULT_ROLE)
